...
#define BUFFER_SIZE 500
    ...
    //-是否要刷新标准输出的缓冲区，一般都要刷新比较好，不会影响到stderr
    if(flushStdout){
        fflush(stdout);
    }
    //-写入标准错误流，并刷新缓冲区，也可以用vfprintf
    //-fputs不支持格式化字符串，vfprintf支持format字符串
    fputs(buf,stderr);
    fflush(stderr);
}
...
void fatal(const char * msg,...){
    va_list argList;
    va_start(argList,msg);
    outputError(FALSE,0,TRUE,msg,argList);
    va_end(argList);
    terminate(TRUE);
}
...
void usageErr(const char * msg,...){
    va_list argList;
    //-stdout和stderr都是输出到屏幕，为了格式化，先刷新srdout的缓冲区
    fflush(stdout);
